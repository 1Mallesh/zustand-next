...
export default function ServiceCard({ id, img, label, onClick }: ServiceCardProps) {
  return (
    <div
      onClick={() => onClick && onClick({ id, img, label })}
      className="cursor-pointer flex flex-col items-center  shadow-sm hover:shadow-md p-4 rounded-xl transition"
    >
      <div
        className={`w-20 h-20 flex items-center justify-center rounded-xl shadow-md ${bgColors[id - 1]}`}
      >
